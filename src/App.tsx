...
const batchColumns: Column[] = [
  ...
  },
  {
    key: 'freeze',
    label: 'Freezer',
    value: (r) => ('freezes_well' in r ? (r.freezes_well ? 'yes' : 'no') : ''),
  },
]
  ...
  return (
    <>
      <tr className="recipe-row" onClick={onToggle} aria-expanded={open}>
        <td>
          <span className="chevron">{open ? '▾' : '▸'}</span> <strong>{recipe.name}</strong>{' '}
          ...
          const title =
            col.key === 'sauce' && sauce
              ...
              : col.key === 'freeze' && 'reheat' in recipe
                ? recipe.reheat
                : undefined
          return <td key={col.key} title={title}>{col.value(recipe)}</td>
        })}
      </tr>
      {open && (
            ...
            <div className="foldout">
              {'reheat' in recipe && (
                <section>
                  <h4>Storage</h4>
                  <p>
                    Fridge: {(recipe as BatchRecipe).fridge_days} days · Freezer:{' '}
                    {(recipe as BatchRecipe).freezes_well ? 'yes' : 'no'} —{' '}
              ...
              {sauce && (
                <section>
                  <h4>Sauce</h4>
                  <p>
                    {sauce.name} ({sauce.technique}) — {sauce.flavor}. {sauce.ingredients}.
                    <em> {sauce.notes}</em>
                  </p>
                </section>
              )}
...
function RecipeTable({
  ...
  const [sort, setSort] = useState<{ key: 'total_time' | 'active_time'; dir: 1 | -1 } | null>(
    null,
  )
  const columns = 'batch_servings' in (recipes[0] ?? {}) ? batchColumns : quickColumns

  function cycleSort(colKey: string) {
    if (colKey !== 'total' && colKey !== 'active') return
    const key = colKey === 'total' ? ('total_time' as const) : ('active_time' as const)
    setSort((s) =>
  ...
  const sorted =
    sort === null
      ? recipes
      : [...recipes].sort((a, b) => (a[sort.key] - b[sort.key]) * sort.dir)
            ...
            {columns.map((col) => (
              <th key={col.key}>
                {col.key === 'total' || col.key === 'active' ? (
                  ...
                  >
                    {col.label}
                    {sort && (col.key === 'total') === (sort.key === 'total_time') && (
        ...
        <tbody>
          {sauces.map((s) => (
            <Fragment key={s.name}>
              ...
              >
                <td>
                  <span className="chevron">{openName === s.name ? '▾' : '▸'}</span>{' '}
